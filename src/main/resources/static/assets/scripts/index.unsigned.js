...

{
    ...
    if (typeof localStorage.getItem('rememberedEmail') === 'string') {
        $loginForm['email'].value = localStorage.getItem('rememberEmail');
    ...
    $loginForm.onsubmit = (e) => {
        ...
        $emailLabel.setValid($loginForm['email'].value.length >= 8 && $loginForm['email'].value.length <= 50, '올바른 이메일을 입력해 주세요.');
        ...
        xhr.onreadystatechange = () => {
            ...
            const [title, content, onclick] = {
                failure: ['로그인', '이메일 혹은 비밀번호가 올바르지 않습니다. 다시 확인해주세요.', ($dialog) => {
                    Dialog.hide($dialog);
                    $loginForm['email'].focus();
                    $loginForm['email'].select();
                }],
                failure_not_verified: ['로그인', `해당 계정의 이메일 인증이 완료되지 않았습니다. 이메일을 확인해 주세요.<br><br>혹시 이메일이 오지 않았다면 인증 링크가 포함된 이메일을 <a href="/user/resend-register-email-token=${$loginForm['email'].value}" target="_blank">다시 전송</a>할 수 있습니다.`, ($dialog) => Dialog.hide($dialog)],
...
$recoverForm.onsubmit = (e) => {
    ...
    if ($recoverForm['mode'].value === '') {
        Dialog.show({
            title: '계정찾기',
            content: '"이메일 찾기" 혹은 "비밀번호 재성정" 중 하나를 선택해주세요',
            buttons: [{
                text: '확인', onclick: ($dialog) => Dialog.hide($dialog)
            }]
        })
    ...
    if ($recoverForm['mode'].value === 'email') {
        ...
        $contactLabel.setValid($recoverForm['contact'].value.length >= 10 && $registerForm['contact'].value.length <= 12);
        ...
        xhr.onreadystatechange = () => {
            ...
            if (xhr.status < 200 || xhr.status >= 300) {
                Dialog.show({
                    title: '오류',
                    content: '요청을 전송하는 도중 오류가 발생하였습니다. 잠시 후 다시 시도해주세요.',
                    buttons: [{
                        text: '확인', onclick: ($dialog) => Dialog.hide($dialog)
                    }]
                })
    ...
    if ($recoverForm['mode'].value === 'password') {
        ...
        $emailLabel.setValid($recoverForm['email'].value.length >= 8 && $registerForm['email'].value.length <= 50);
        ...
        xhr.onreadystatechange = () => {
            ...
            const [title, content, onclick] = {
                ...
                success: ['비밀번호 재 설정', `입력하신 이메일로 비밀번호를 재설정 할 수 있는 링크를 포함한 메일을 전송하였습니다.<br><br>확인을 클릭하면 로그인 화면으로 돌아갑니다.`, ($dialog) => {
                    Dialog.hide($dialog);
                    $cover.hide();
                    $recoverForm.hide();
                }],
